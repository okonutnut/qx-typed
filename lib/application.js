...
class AutoSchedulerWindow extends qx.ui.window.Window {
    ...
    __loadData() {
        return __awaiter(this, void 0, void 0, function* () {
            const [subjectsResult, facultySubjectsResult, facultyResult, roomsResult, schedulesResult, semestersResult] = yield Promise.all([
    ...
    __filterByActiveSemester() {
        const active = this.__semesters.find((s) => s.isActive);
        if (active) {
            this.__existingSchedules = this.__existingSchedules.filter((s) => s.semesterId === active.id);
        }
    }
    __generateSchedules() {
