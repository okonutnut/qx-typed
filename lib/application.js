...
class Navbar extends qx.ui.container.Composite {
    constructor(pageTitle, onToggleSidebar) {
        ...
        // SIDEBAR TRIGGER
        const collapseSidebarBtn = new BsButton("", new InlineSvgIcon("panel-left-close", 8), "btn-sm-icon-ghost p-1", "ghost");
    ...
    __createActionsMenuButton(label, action) {
...
class Sidebar extends qx.ui.container.Composite {
    constructor(sidebarItems, initialActiveLabel) {
        ...
        const footer = new BsSidebarAccount("Ronan Berder", "@hunvreus", "resource/app/user.png", "RB");
...
class BsButton extends qx.ui.basic.Atom {
    constructor(text, icon, className, variant) {
        super();
...
class BsSidebarAccount extends qx.ui.basic.Atom {
    ...
    __placeMenuPopup() {
        ...
        if (this.__collapsed) {
            ...
            top = Math.min(Math.max(8, Math.round(triggerRect.top)), Math.max(8, viewportHeight - popupRect.height - 8));
        }
        else {
    ...
    __buildMenuWidgets() {
        ...
        this.__menuContainer.add(this.__createMenuButton("Profile", "⇧⌘P", "profile"));
        this.__menuContainer.add(this.__createMenuButton("Settings", "⌘S", "settings"));
        ...
        this.__menuContainer.add(this.__createMenuButton("Log out", "⇧⌘Q", "logout"));
    }
    __createMenuButton(label, shortcut, action) {
        const button = new BsButton(`${label}    ${shortcut}`, undefined, "btn-sm-outline", "outline");
        button.setAllowGrowX(true);
        button.setMarginTop(-2);
        button.setMarginBottom(-2);
        button.onClick(() => {
...
class ButtonsPage extends qx.ui.container.Composite {
    constructor() {
        ...
        const outlineBtn = new BsButton("Outline", null, "btn-sm-outline", "outline");
        const toastBtn = new BsButton("Show Toast", null, "btn-sm-outline", "outline");
        const alertDialogBtn = new BsButton("Open Alert Dialog", null, "btn-sm-outline", "outline");
...
class FormPage extends qx.ui.container.Composite {
    constructor() {
        super(new qx.ui.layout.VBox(3));
        const setError = (errorLabel, message) => {
            const text = (message !== null && message !== void 0 ? message : "").trim();
        ...
        const passwordInput = new BsInput("", "Enter your password", "input-bordered");
        ...
        const countryInput = new BsInput("", "Enter your country", "input-bordered");
        ...
        const bioInput = new BsInput("", "Tell us about yourself", "input-bordered");
        ...
        const genderSelect = new BsSelect(["Man", "Woman", "Genderqueer/Non-Binary", "Prefer not to disclose"], "select-bordered");
        ...
        const resetButton = new BsButton("Reset", undefined, "btn-sm-outline", "outline");
        sendButton.onClick(() => {
            let hasError = false;
...
class MainPage extends qx.ui.container.Composite {
    constructor() {
        ...
        this.setLayout(new qx.ui.layout.Grow());
        this.setBackgroundColor(AppColors.background());
        const center = new qx.ui.container.Composite(new qx.ui.layout.VBox(12).set({ alignY: "middle" }));
        ...
        const subtitle = new qx.ui.basic.Label("Your workspace is ready. Use the sidebar to explore components and pages.");
